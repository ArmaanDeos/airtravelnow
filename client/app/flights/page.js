...
const FlightResultPage = () => {
  ...
  const [sortOption, setSortOption] = useState("recommended"); // State to hold the selected sort option
  ...
  // Function to sort the flight results
  const sortResults = (option) => {
    switch (option) {
      case "price-low":
        return [...results].sort((a, b) => {
          const priceA = a.price?.total ?? 0; // Default to 0 if price is undefined
          const priceB = b.price?.total ?? 0; // Default to 0 if price is undefined
      ...
      case "duration":
        return [...results].sort(
          (a, b) => a.slices[0].duration - b.slices[0].duration // Sort by duration (shortest duration)
        );
      case "recommended":
      default:
        return results; // Default: no sorting (recommended)
    }
  };

  const sortedResults = sortResults(sortOption); // Get sorted results based on selected option
      ...
      <div className="flex flex-col lg:flex-row max-w-9xl mx-auto min-h-screen pt-[120px]">
        ...
        <div className="flex-1 p-4 sm:p-6 lg:p-8">
          ...
          <FlightListing
            ...
            results={sortedResults} // Pass the sorted results to the listing
          />
        </div>
      </div>
