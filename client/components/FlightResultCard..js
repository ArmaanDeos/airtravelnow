...
const FlightListing = ({
  ...
  const rawOffers = (results || []).filter((offer) => {
    ...
    const inbound = slices[1]; // if exists (round trip)
    ...
    const matchesInbound = return_date
      ...
      : true; // if one-way, skip inbound check
  ...
  const flights = currentFlights.map((offer) => {
    const slices = offer.slices || [];
    const outbound = slices[0];
    ...
    return {
      ...
      outbound: {
        ...
        airline: outboundSegment?.marketing_carrier?.name || "Unknown Airline",
        flight_number:
  ...
  if (loading)
    return (
      <div className="text-center text-orange-600">Loading flights...</div>
    );
  if (error)
    ...
    <div className="space-y-6">
      {flights.map((flight) => (
        ...
        >
            ...
            <div className="flex-1 space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <div className="text-xl font-bold text-orange-800">
                    {new Date(flight.outbound.departure).toLocaleTimeString(
                      [],
                      {
                        hour: "2-digit",
                        minute: "2-digit",
                ...
                <div className="flex flex-col items-center justify-center space-y-1">
                  <div className="text-sm text-gray-500">
                    ...
                    {flight.outbound.duration?.replace("PT", "").toLowerCase()}
                  </div>
                ...
                <div>
                  <div className="text-xl font-bold text-orange-800">
                    {new Date(flight.outbound.arrival).toLocaleTimeString([], {
                      hour: "2-digit",
                      minute: "2-digit",
                    })}
                  </div>
              ...
              {flight.inbound && (
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 border-t pt-4">
                  <div>
                    <div className="text-xl font-bold text-orange-800">
                      {new Date(flight.inbound.departure).toLocaleTimeString(
                        [],
                        {
                          hour: "2-digit",
                          minute: "2-digit",
                  ...
                  <div className="flex flex-col items-center justify-center space-y-1">
                    <div className="text-sm text-gray-500">
                      ...
                      {flight.inbound.duration?.replace("PT", "").toLowerCase()}
                    </div>
                    <div className="w-full border-t border-dashed relative py-2">
                      <div className="absolute top-1/2 left-0 right-0 border-t border-orange-300"></div>
                      <Plane className="h-4 w-4 text-orange-400 mx-auto bg-white relative z-10" />
                  ...
                  <div>
                    <div className="text-xl font-bold text-orange-800">
                      {new Date(flight.inbound.arrival).toLocaleTimeString([], {
                        hour: "2-digit",
                        minute: "2-digit",
                      })}
                    </div>
          ...
          {expandedId === flight.id && (
            <div className="mt-4 bg-orange-50 p-4 rounded-lg text-sm space-y-4 border border-orange-100">
              <div>
                <h4 className="font-semibold mb-1">👥 Passenger Information</h4>
                <p>
